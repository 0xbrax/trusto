...
export const getSolanaConnection = async () => {
    if (!cachedConnection) {
        cachedConnection = await new Connection(clusterApiUrl(process.env.SOLANA_NETWORK || process.env.NEXT_PUBLIC_SOLANA_NETWORK), "finalized");
    }
...
export const calculateHash = (data) => {
    const sortedObj = sortObjectAlphabetically(data);
...
export const openBlockchainExplorerAddress = (address) => {
